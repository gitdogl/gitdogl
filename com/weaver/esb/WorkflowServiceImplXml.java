...
//
///**
// * @author ludifu 流程xml接口,提供创建，提交，删除流程等方法
// * @version 1.0
// */
...
//    private final static weaver.workflow.logging.Logger log = weaver.workflow.logging.LoggerFactory.getLogger(WorkflowServiceImpl.class);
//
//
//    /**
//     * 删除流程
...
//        try {
//            log.error("webserviceXml删除流程,requestId:" + requestid + "用户ID:" + userid);
...
//        String where = " where t1.requestid=t2.requestid ";
//        where += " and t3.id=t2.workflowid and (t3.isvalid=1 or t3.isvalid=3) ";
//        where += " and t2.usertype = 0 and t2.userid = " + userid;
...
//            }
//            if (pageNo == 1)
//                sql = select + " " + fields + " " + from + " " + where + " " + orderby + " limit " + endResult;
//            else
//                sql = " select * from ( select * from ( " + select + " " + fields + " " + from + " " + where + " " + orderby + " limit " + firstResult + ") tbltemp1 " + orderby1 + " limit " + endResult + "  ) tbltemp2 " + orderby2 + " limit " + endResult;
...
//     *
//     * @param wri  WorkflowRequestInfo对象
//     * @param type create:创建流程  submit:提交流程
//     * @return RequestInfo
//     */
//    private RequestInfo toRequestInfo(WorkflowRequestInfo wri, String type) throws Exception {
...
//            requestInfo = requestService.getRequest(Util.getIntValue(wri.getRequestId()));
//
...
//            requestInfo.setDescription(Util.processHtmlUnicode(wri.getRequestName()));//加强处理:流程名称为html的unicode编码的问题
//        }
//        //提交流程不传requestName，取原有的requestName
//        else if (type.equals("submit")) {
//            if (wri.getRequestName() == null || "".equals(wri.getRequestName())) {
//                RecordSet recordSet = new RecordSet();
//                recordSet.executeQuery("select requestName from workflow_requestbase where requestId=?", wri.getRequestId());
//                if (recordSet.next()) {
//                    wri.setRequestName(recordSet.getString(1));
//                }
//            }
//            requestInfo.setDescription(Util.processHtmlUnicode(wri.getRequestName()));//加强处理:流程名称为html的unicode编码的问题
//        }
...
//            WorkflowRequestTableRecord[] wrtrs = wmti.getRequestRecords();
//            if (wrtrs != null && wrtrs[0] != null) {
//                if (wrtrs[0].getWorkflowRequestTableFields() != null) {
//                    for (int i = 0; i < wrtrs[0].getWorkflowRequestTableFields().length; i++) {
//                        WorkflowRequestTableField wrtf = wrtrs[0].getWorkflowRequestTableFields()[i];
//                        if (wrtf != null) {
...
//                            cell.setType(wrtf.getFieldType());//明细附件字段需要有这个
//                            cells.add(cell);
//                        }
//
//                    }
...
//        if (isremark == 1 || isremark == 2 || isremark == 8 || isremark == 9 || isremark == 7)
...
//            //WorkflowRequestTableField wrtf = WorkflowServiceUtil.getWorkflowRequestField(wid, fieldid, fieldname, fieldvalue, fieldhtmltype, fieldtype, fielddbtype, fieldlable, fieldformname, tmpindex, languageid, isview, isedit, ismand, user, specialfield, docfieldids, requestid==0,requestid);
...
//            String syspath = "";
//            rs.executeSql(" select * from workflow_nodehtmllayout where type=2 and isactive=1 and workflowid = " + workflowid + " and nodeid = " + nodeid);
//            while (rs.next()) {
...
//        workflowId = "" + Util.getIntValue(workflowId);
//        requestId = "" + Util.getIntValue(requestId);
...
//        }
//
//        String[] suffixs = new String[]{"<br/><br/><span style=\"font-size:11px;color:#666;\">" + SystemEnv.getHtmlLabelName(83643, 7) + "</span>",
//                "<br/><br/><span style=\"font-size:11px;color:#666;\">" + SystemEnv.getHtmlLabelName(83647, 7) + "</span>",
//                "<br/><br/><span style=\"font-size:11px;color:#666;\">" + SystemEnv.getHtmlLabelName(83649, 7) + "</span>",
//                "<br/><br/><span style=\"font-size:11px;color:#666;\">" + SystemEnv.getHtmlLabelName(83652, 7) + "</span>"};
//
...
//        String result = "0.00";
//        try {
//            resourceId = "" + Util.getIntValue(resourceId);
//            ResourceComInfo rci = new ResourceComInfo();
//            DepartmentComInfo dci = new DepartmentComInfo();
//            RecordSet rs = new RecordSet();
...
//        workflowid = "" + Util.getIntValue(workflowid);
//        requestid = "" + Util.getIntValue(requestid);
//        nodetype = "" + Util.getIntValue(nodetype);
//        Map map = new HashMap();
//        RecordSet recordSet = new RecordSet();
...
//                String ri = "" + Util.getIntValue(requestids[i]);
//                requestidstr += "," + ri;
//            }
...
//            RecordSet rs = new RecordSet();
//            requestid = "" + Util.getIntValue(requestid);
//            userid = "" + Util.getIntValue(userid);
//            //获得当前的日期和时间
...
//            rs.executeSql("select phraseShort,phraseDesc from sysPhrase where hrmid =" + user.getUID() + " or hrmid = -1 order by dsporder,id ");
//            while (rs.next()) {
...
//     * @return String[]
//     */
//    public String[] getHendledWorkflowRequestList(int pageNo, int pageSize, int recordCount, int userid, String[] conditions) {
//        if (pageNo < 1)
//            pageNo = 1;
//        String select = " select distinct ";
//        String fields = " t1.createdate,t1.createtime,t1.creater,t1.currentnodeid,t1.currentnodetype,t1.lastoperator,t1.creatertype,t1.lastoperatortype,t1.lastoperatedate,t1.lastoperatetime,t1.requestid,t1.requestname,t1.requestlevel,t1.workflowid,t2.receivedate,t2.receivetime ";
...
//     * @return String
//     */
//    public String getWorkflowRequest4split(int requestid, int userid, int fromrequestid, int pagesize) {
//        try {
...
//
//    /**
//     * 将RequestInfo转换为WorkflowRequestInfoXML
//     *
//     * @param ri            请求信息对象
//     * @param userid        用户 id
//     * @param fromrequestid 来源请求id
...
//     * @return String
//     */
//    private String getFromRequestInfo(RequestInfo ri, int userid, int fromrequestid, int pagesize) throws Exception {
...
//            lastOperateTime = Util.null2String(rs.getString("lastoperatedate")) + " " + Util.null2String(rs.getString("lastoperatetime"));
//        }
//
...
//        wri.setLastOperateTime(lastOperateTime);
//
...
//                    //更新为已查看
//                    String updateView = "UPDATE workflow_currentoperator set viewtype =-2,firstviewdate =operatedate , firstviewtime =operatetime ,viewdate =operatedate ,viewtime =operatetime where requestid =? and userid=?";
//                    rs.executeUpdate(updateView, ri.getRequestid(), userid);
//
...
//        String xmlbean = (String) xmlutil1.objToXml(wri);
//        return xmlbean;
//    }
//
//    /**
...
//        try {
//            log.error("webserviceXml创建流程,创建人:" + userId + "创建报文参数如下:" + xml);
//            WorkflowRequestInfo wri = (WorkflowRequestInfo) xmlutil1.xmlToObject(xml);
//            wri = getActiveWorkflowRequestInfo(wri);
//            RequestInfo ri = toRequestInfo(wri, "create");
...
//     * @return String[]
//     */
//    public String[] getToDoWorkflowRequestList(int pageNo, int pageSize, int recordCount, int userid, String[] conditions) {
//        if (pageNo < 1)
//            pageNo = 1;
//        String select = " select distinct ";
//        String fields = " t1.createdate,t1.createtime,t1.creater,t1.currentnodeid,t1.currentnodetype,t1.lastoperator,t1.creatertype,t1.lastoperatortype,t1.lastoperatedate,t1.lastoperatetime,t1.requestid,t1.requestname,t1.requestlevel,t1.workflowid,t2.receivedate,t2.receivetime ";
...
//     */
//    private String[] getWorkflowRequestList(String sql) {
//        RecordSet rs = new RecordSet();
//        RecordSet rs1 = new RecordSet();
...
//
//                String xml = (String) xmlutil1.objToXml(wri);
//                wris.add(xml);
//            }
//        } catch (Exception e) {
//            e.printStackTrace();
//            writeLog(e);
//        }
//        String[] wriarrays = new String[wris.size()];
//        for (int i = 0; i < wris.size(); i++)
//            wriarrays[i] = (String) wris.get(i);
//        return wriarrays;
//    }
//
//
...
//     * @return String[]
//     */
//    public String[] getMyWorkflowRequestList(int pageNo, int pageSize, int recordCount, int userid, String[] conditions) {
//        if (pageNo < 1)
//            pageNo = 1;
//        String select = " select distinct ";
//        String fields = " t1.createdate,t1.createtime,t1.creater,t1.currentnodeid,t1.currentnodetype,t1.lastoperator,t1.creatertype,t1.lastoperatortype,t1.lastoperatedate,t1.lastoperatetime,t1.requestid,t1.requestname,t1.requestlevel,t1.workflowid,t2.receivedate,t2.receivetime ";
...
//     * @return String[]
//     */
//    public String[] getProcessedWorkflowRequestList(int pageNo, int pageSize, int recordCount, int userid, String[] conditions) {
//        if (pageNo < 1)
//            pageNo = 1;
//        String select = " select distinct ";
//        String fields = " t1.createdate,t1.createtime,t1.creater,t1.currentnodeid,t1.currentnodetype,t1.lastoperator,t1.creatertype,t1.lastoperatortype,t1.lastoperatedate,t1.lastoperatetime,t1.requestid,t1.requestname,t1.requestlevel,t1.workflowid,t2.receivedate,t2.receivetime ";
...
//     * @return String[]
//     */
//    public String[] getCCWorkflowRequestList(int pageNo, int pageSize, int recordCount, int userid, String[] conditions) {
//        if (pageNo < 1)
//            pageNo = 1;
//        String select = " select distinct ";
//        String fields = " t1.createdate,t1.createtime,t1.creater,t1.currentnodeid,t1.currentnodetype,t1.lastoperator,t1.creatertype,t1.lastoperatortype,t1.lastoperatedate,t1.lastoperatetime,t1.requestid,t1.requestname,t1.requestlevel,t1.workflowid,t2.receivedate,t2.receivetime ";
...
//
//
//    public String[] getAllWorkflowRequestList(int pageNo, int pageSize, int recordCount, int userid, String[] conditions) {
//        if (pageNo < 1)
//            pageNo = 1;
//        String select = " select distinct ";
//        String fields = " t1.createdate,t1.createtime,t1.creater,t1.currentnodeid,t1.currentnodetype,t1.lastoperator,t1.creatertype,t1.lastoperatortype,t1.lastoperatedate,t1.lastoperatetime,t1.requestid,t1.requestname,t1.requestlevel,t1.workflowid,t2.receivedate,t2.receivetime ";
...
//     * @return String
//     */
//    public String getCreateWorkflowRequestInfo(int workflowid, int userid) {
//        WorkflowRequestInfo wri = new WorkflowRequestInfo();
//        try {
//            RecordSet rs = new RecordSet();
//            char flag = Util.getSeparator();
...
//     * @param userid      用户id
//     * @param conditions  自定义条件
//     * @return getCreateWorkflowTypeList
//     */
//    public String[] getCreateWorkflowTypeList(int pageNo, int pageSize, int recordCount, int userid, String[] conditions) {
//        if (pageNo < 1)
//            pageNo = 1;
...
//                String xml = (String) xmlutil1.objToXml(wbi);
//                wbis.add(xml);
//            }
//        } catch (Exception e) {
//            e.printStackTrace();
//            writeLog(e);
//        }
//        String[] wbiarrays = new String[wbis.size()];
//        for (int i = 0; i < wbis.size(); i++)
//            wbiarrays[i] = (String) wbis.get(i);
//
//        return wbiarrays;
//    }
//
...
//     * @param recordCount  总条数
//     * @param userid       用户id
//     * @param workflowType 数据类型
//     * @param conditions   自定义条件
//     * @return
//     */
//    public String[] getCreateWorkflowList(int pageNo, int pageSize, int recordCount, int userid, int workflowType, String[] conditions) {
//        List wbis = new ArrayList();
//        try {
//            WorkTypeComInfo workTypeComInfo = new WorkTypeComInfo();
//            WorkflowComInfo workflowComInfo = new WorkflowComInfo();
...
//     * @return
//     */
//    public String submitWorkflowRequest(String xml, int requestid, int userid, String type, String remark) {
//        String result = "error";
//        try {
//            log.error("webserviceXml提交流程,requestId:" + requestid + "用户ID:" + userid + " type:" + type + " 提交报文参数如下:" + xml);
//            WorkflowRequestInfo wri = (WorkflowRequestInfo) xmlutil1.xmlToObject(xml);
...
//                if (wri != null && wri.getRemark() != null) remark = wri.getRemark();
//            }
//            log.error("webserviceXml提交流程,remark:" + remark);
//            RequestInfo ri = this.toRequestInfo(wri, "submit");
...
//                if (requestService.nextNodeBySave(ri, requestid, userid))
//                    result = "success";
//                else
//                    result = "failed";
//            }
...
//        //select t.activeversionid from workflow_base  t where id=流程id
//        workflowid = "" + Util.getIntValue(workflowid);
...
//     * @return WorkflowRequestInfo
//     */
//    private WorkflowRequestInfo getActiveWorkflowRequestInfo(WorkflowRequestInfo wri) {
//        WorkflowComInfo wfComInfo = new WorkflowComInfo();
//        WorkTypeComInfo WorkflowTypeComInfo = new WorkTypeComInfo();
//
//        WorkflowBaseInfo oldWorkflowBaseInfo = wri.getWorkflowBaseInfo();
//        String activewfid = getActiveversionid(oldWorkflowBaseInfo.getWorkflowId());
//        oldWorkflowBaseInfo.setWorkflowName(wfComInfo.getWorkflowname(activewfid));
//        oldWorkflowBaseInfo.setWorkflowTypeId(wfComInfo.getWorkflowtype(activewfid));
//        oldWorkflowBaseInfo.setWorkflowTypeName(WorkflowTypeComInfo.getWorkTypename(wfComInfo.getWorkflowtype(activewfid)));
...
//     * @param recipients  目标用户】
//     * @param remark      签字意见
//     * @param forwardflag 类型
//     * @return String 转办结果或意见征询结果
//     */
//    public String forward2WorkflowRequest(int requestid, String recipients,
//                                          String remark, int userId, String forwardflag) {
//        if (requestid <= 0) {
//            return "-1";
//        }
//        if (userId <= 0) {
...
//            return "-3";
//        }
//        if ("2".equals(forwardflag) || "3".equals(forwardflag)) {
//            //2 意见征询  3 转办
//            return requestService.forwardFlow(requestid, userId, recipients, remark, "", forwardflag);
//        } else {
//            return "-4";
//        }
//    }
...
//     * @param userid    当前用户
//     * @param remark    意见
//     * @return String 结果 1表示成功 -1表示失败
//     */
//    public String givingOpinions(int requestid, int userid, String remark) {
...
//            return "error";
//        }
//        return requestService.givingOpinions(requestid, userid, remark);
//    }
//
//
//    /**
...
//     * @param requestid 流程请求ID
//     */
//    public String doForceOver(int requestid, int userId) {
//        log.error("webserviceXml强制归档,requestId:" + requestid + "用户ID:" + userId);
//        // TODO Auto-generated method stub
//        if (requestid <= 0) {
//            return "error";
//        }
...
//            return "error";
//        }
//        return requestService.doForceOver(requestid, userId, "");
//    }
//
//    /**
//     * 根据人力资源表对应的字段对应的值，查找人员id
//     *
...
//        if (Util.null2String(filedValue).equals("")) {
//            return "-1";//filedValue为空，返回-1
//        }
//        if (Util.null2String(filedType).equals("")) {
//            return "-2";//filedType为空，返回-2
//        }
//        RecordSet rs = new RecordSet();
//        rs.executeQuery("select id from HrmResource where " + filedType + "=? and status<4 ", filedValue);
//        if (!rs.next()) {
//            return "0";//找不到人，返回0
//        }
//
//        return Util.null2s(rs.getString("id"), "0");//正常返回
//    }
//
//}
