 ...
 * 
 */
public class ERPCurrentDateSyn extends BaseCronJob {

	/**
	 * 日志输出
	 */
	private Log log = LogFactory.getLog(ERPCurrentDateSyn.class.getName());
	...
	public void execute() {
		...
		try {
			...
			String sql = " select requestid||'_'||id  itemid  from workflow_currentoperator  where  isremark in (2,4) and operatedate>='"+TimeUtil.dateAdd(TimeUtil.getCurrentDateString(), -7)+"'";
			...
			String sqldata = " select itemid from epintegratedinfo ";
