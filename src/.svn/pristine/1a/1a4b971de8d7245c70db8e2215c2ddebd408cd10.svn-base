...
public class OAStatementsDao {
     ...
     * @return
     */
    public OAStatementsBill getOAStatementsBill(String billid) {
        OAStatementsBill oaStatementsBill = new OAStatementsBill();
        StringBuffer sqlBuff = new StringBuffer("select * from uf_jzd where id = " + billid);
    ...
    public List<OAStatementsDto2> getOAStatementsDto2List(String mainid) {
        List<OAStatementsDto2> oaStatementsDto2List = new ArrayList<>();
        StringBuffer sqlBuff = new StringBuffer("select * from uf_jzd_dt2 where mainid = ").append(mainid);
        rs.execute(sqlBuff.toString());
        while (rs.next()) {
