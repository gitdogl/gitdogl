...
public class jypz extends ToolUtil {
    ...
    private Map<String, String> handle() {
        ...
        try {
            ...
            if (cmap != null) {
                ...
                //主表贷方字段配置集合
                List<Map<String, Object>> zb_loan_list = (List<Map<String, Object>>) cmap.get("zb_loan_list");
                ...
                if (mainkeyid > 0) {
                    if ("".equals(detailIndex) || "0".equals(detailIndex)) {//说明所有借贷方皆来自主表记录
                        ...
                        //主表贷方
                        if (zb_loan_list != null && zb_loan_list.size() > 0) {
                            JSONObject zb_loan_obj = vu.genarate_json(baseArray, zb_loan_list, rs, null, 1, "", rowIndex++);
                            if (zb_loan_obj != null && Util.getDoubleValue(Util.null2String(zb_loan_obj.get("WRBTR")), 0) != 0) {
                                detailArray.add(zb_loan_obj);
                            }
                        }
                        ...
                        for (String detailRow : detailindexArray) {//遍历明细序列
                            ...
                            if (rs_detail.executeQuery(select_detail, mainkeyid)) {
                                ...
                                */
                                    ...
                                    //主表贷方
                                    if (zb_loan_list != null && zb_loan_list.size() > 0) {
                                        JSONObject zb_loan_obj = vu.genarate_json(baseArray, zb_loan_list, rs, rs_detail, 1, dtTableName, rowIndex++);
                                        if (zb_loan_obj != null && Util.getDoubleValue(Util.null2String(zb_loan_obj.get("WRBTR")), 0) != 0) {
                                            detailArray.add(zb_loan_obj);
                                        }
                                    }
...
//                                    }
                    ...
                    if (com_loan_list != null && com_loan_list.size() > 0) {
                        JSONObject com_loan_obj = vu.genarate_json(baseArray, com_loan_list, rs, null, 1, "", rowIndex++);
                        if (com_loan_obj != null && Util.getDoubleValue(Util.null2String(com_loan_obj.get("WRBTR")), 0) != 0) {
                            detailArray.add(com_loan_obj);
                        }
                    }

                    this.writeNewDebuggerLog(className, "ITEMS:" + detailArray.toString());

                    com.alibaba.fastjson.JSONArray resultArray = new com.alibaba.fastjson.JSONArray();
                    for(int i=0;i<detailArray.size();i++){
                        net.sf.json.JSONObject detailobj=detailArray.getJSONObject(i);
                        int j=i+1;
                        detailobj.put("BUZEI",j+"");
                        resultArray.add(detailobj);
                    }



                    jsonobj.put("ITEMS", resultArray);
                    ...
                    if (resultobj != null) {
                        ...
                        if ("100".equals(code)) {
                            ...
                            String EV_STATUS = dataobj.getString("STATUS");
                            if ("S".equals(EV_STATUS)) {//说明成功
                                vlog.setResult(1);

                                cansubmit = true;
                                //获取返回成功的凭证号
                                String EV_BELNR = Util.null2String(dataobj.getString("ORDER"));
                                String EV_MESSAGE = Util.null2String(dataobj.getString("MESSAGE"));
...
class ToolUtil extends BaseBean {
    ...
    public void writeNewLog(String o, String s) {
        try {
            ...
            String folder = GCONST.getRootPath() + "log" + File.separatorChar + "XT";
...
class VoucherUtil extends ToolUtil {
    ...
    public Map<String, Object> getconfig(String workflowid, String cusparam) {
        ...
        if (mainkeyid > 0) {
            ...
            //中转贷方字段配置集合
            List<Map<String, Object>> zb_loan_list = new ArrayList<Map<String, Object>>();
            ...
            c_map.put("zb_loan_list", zb_loan_list);
    ...
    JSONObject genarate_json(String[] baseArray, List<Map<String, Object>> datalist, RecordSet rs, RecordSet rs_detail, int jsonType, String dtTable, int index) {
        ...
        if (datalist != null && datalist.size() > 0) {
            for (Map<String, Object> detailmap : datalist) {
                ...
                if (jsonname.indexOf("ITEMS.") > -1) {
                    //明细字段去除明细标识
                    jsonname = jsonname.substring(6, jsonname.length());
                }
